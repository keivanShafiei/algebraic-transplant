...
def load_gnn_surrogate(checkpoint_path, solver, device):
    ...
    try:
        ...
        model = NeuralOperator(
            in_dim=2, hidden_dim=128, out_dim=2,
            num_layers=6, num_nodes=solver.N,
        ).to(device)
...
def run_solver_direct(solver, Re, x0, label, n_max=100, verbose=False):
    ...
    a, b_full, iters, mom_hist, div_hist = solver.solve(
        Re=Re, x0=x0, tau_mom=1e-2, tau_mass=1e-4,
        n_max=n_max, use_iterative=True, mom_tol=1e-6,
        verbose=verbose,
    )
    ...
    return {
        'a': a, 'b': b_full, 'iters': iters,
        'mom_hist': mom_hist, 'div_hist': div_hist,
        'converged': converged, 'time_s': elapsed,
    }


def run_solver_continuation(solver, Re_target, x0, label, n_max_per_step=200, verbose=False):
    start = time.time()
    a, b_full, total_iters, mom_hist, div_hist = solver.solve_continuation(
        Re_target=Re_target, x0=x0, tau_mom=1e-2, tau_mass=1e-4,
        n_max_per_step=n_max_per_step, use_iterative=True, mom_tol=1e-6,
        verbose=verbose,
    )
    elapsed = time.time() - start
    converged = (mom_hist[-1] < 1e-2) and (div_hist[-1] < 1e-4) if mom_hist else False
    ...
    return {
        'a': a, 'b': b_full, 'iters': total_iters,
        'mom_hist': mom_hist, 'div_hist': div_hist,
        'converged': converged, 'time_s': elapsed,
    }
...
def main():
    ...
    print("INFO: ############################################################")
    print("INFO: Using continuation solver (Re=10->20->30->50->75->100->150->200->250->300->350->400->450->500)")
    print("INFO: Paper's Table 10 reports iteration counts with continuation.")

    res_cold_500 = run_solver_continuation(
        solver, 500.0, None, "Cold start continuation (Re=500)", n_max_per_step=200
    )
    ...
    res_df_500 = run_solver_continuation(
        solver, 500.0, div_free_500, "Div-free zero continuation (Re=500)", n_max_per_step=200
    )
    ...
    if surrogate_fn is not None:
        ...
        res_surrogate_500 = run_solver_continuation(
            solver, 500.0, a_no, "Surrogate continuation (Re=500)", n_max_per_step=200
        )
    ...
    print("INFO:   Expected: ~0 (zero field is trivially div-free)")
    ...
    if res_surrogate_500:
        ...
        print("INFO: Paper claim: 4.2x")
    print(f"INFO: [Re=500] Pure Picard cold: {res_picard_cold['iters']} iters, converged={res_picard_cold['converged']}")

    results = {
        "Re": Re_test, "N": N, "device": str(device),
        "Re_100": {
            "cold_iters": res_cold_100['iters'], "cold_converged": res_cold_100['converged'],
            "zero_df_iters": res_df_100['iters'], "zero_df_converged": res_df_100['converged'],
        },
        "Re_500_continuation": {
            "cold_iters": res_cold_500['iters'], "cold_converged": res_cold_500['converged'],
            "zero_df_iters": res_df_500['iters'], "zero_df_converged": res_df_500['converged'],
        ...
        "Re_500_pure_picard": {
            "cold_iters": res_picard_cold['iters'], "cold_converged": res_picard_cold['converged'],
        },
        "paper_claims": {
