...
CRITICAL FIX (Phase 1): The fixture now correctly loads G_int from fixed_G.pt
(which was generated by the fixed generate_data.py). The previous fixture
implicitly assumed G_full was being used, but fixed_G.pt now contains G_int.
"""
...
def test_divergence_zero_at_interior(setup_boundary_invariance):
    ...
    CRITICAL FIX: This test now correctly uses proj.G (which is G_int loaded
    from fixed_G.pt) to compute divergence. The projection layer enforces
    G_int @ a_NO = 0 by construction (Theorem 2), so the residual should be
    at the float32 precision floor (~4e-5).

    Previously, if G_full was used instead of G_int, the divergence at
    boundary rows would be large because the projection only enforces the
    constraint on interior rows.
    """
    proj, interior_mask, boundary_mask = setup_boundary_invariance
    ...
    # Compute divergence using G_int (proj.G)
    # The projection solves: (G_int G_int^T + eps*I) q = G_int a_hat
    # Then: a_NO = a_hat - G_int^T q (on interior DOFs)
    # Therefore: G_int @ a_NO = G_int @ a_hat - G_int @ G_int^T q
    #                         = G_int @ a_hat - (L_int) q
    #                         ≈ G_int @ a_hat - G_int @ a_hat = 0 (to eps precision)
    div = proj.G @ a_NO
    ...
    assert div_norm < 5e-5, (
        ...
        f"Make sure fixed_G.pt contains G_int (interior-restricted), not G_full."
    )
